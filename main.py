...
mouse_game_coords = (leftMouseX, 173, leftMouseX+(mouseBoxSize*numBoxesX), 173+(mouseBoxSize*numBoxesY))
game_coords = (leftScreenX, 340, leftScreenX+(boxSize*numBoxesX), 340+(boxSize*numBoxesX))

#screen = ImageGrab.grab(bbox=game_coords)
#screen.show()

topLeft = [leftMouseX+int(mouseBoxSize/2), 173+int(mouseBoxSize/2)]
reset = [521, 143]
...
def convertPointToNum(screen, x, y):
    color = screen[x, y+7] #2 is offcenter
    if closeEnough(color, (20, 130, 20)):
        return "2"
    color = screen[x, y-10] #7 is offcenter
    if closeEnough(color, (94, 94, 94)):
        return "7"
    color = screen[x, y]
    if closeEnough(color, (189, 189, 189)):
        topColor = screen[x-17, y-17]
    ...
    if closeEnough(color, (222, 94, 94)):
        return "3"
    if closeEnough(color, (228, 48, 48)) and closeEnough(screen[x, y+10], (0, 0, 0)): #flag is red on top and black on bottom
        return "f"
...
while True:
    ...
    if mouse_game_coords[0] < mouse_pos.x < mouse_game_coords[2] and mouse_game_coords[1] < mouse_pos.y < mouse_game_coords[3]:
        ...
        while True:
            ...
            for row in range(numBoxesY):
                ...
                for column in range(numBoxesX):
                    gameRow.append(convertPointToNum(screen, nowReading[0], nowReading[1]))
                    nowReading[0] += 40
                nowReading[1] += 40
        ...
        if gameOver:
            ...
            print(sleepyTime)
            print(screenShotTime)
            print(timesScreenshot)
            screen = ImageGrab.grab(bbox=game_coords)
            #screen.show()
            screen = screen.load()
            ...
            for row in range(numBoxesY):
                gameRow = []
                for column in range(numBoxesX):
                    if convertPointToNum(screen, nowReading[0], nowReading[1]) != gameStatus[row][column]:
                        hardBreak = False
                    nowReading[0] += 40
                nowReading[1] += 40
                nowReading[0] = originalColumn
